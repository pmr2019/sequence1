...
public class ShowListActivity extends AppCompatActivity implements View.OnClickListener {
        ...
        if (prefs != null) {
            for (int i = 0; i < prefs.size(); i++) {
                if (prefs.get(i).getAsJsonObject().get("pseudo").getAsString() == pseudo) {
                    liste = new Gson().fromJson(prefs.get(i), ListeToDo.class);
                }
            }
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btnOK:
                String nomItem = edtListe.getText().toString();
                mDataset.add(nomItem);
                mAdapter.notifyDataSetChanged();
                edtListe.getText().clear();

                // sauvegarder dans les sharedpreferences les nouvelles listes
                ItemToDo nouvelItem = new ItemToDo(nomItem);
                liste.getItems().add(nouvelItem);
                SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(this);
                SharedPreferences.Editor editor = settings.edit();
                editor.clear();
                JsonArray prefs = new Gson().fromJson(settings.getString("prefs", null), JsonArray.class);
                ...
                for (int i = 0; i < profil.getListes().size(); i++) {
                    if (profil.getListes().get(i).getTitre() == liste.getTitre()) {
                        profil.getListes().set(i, liste);
                    }
                }
